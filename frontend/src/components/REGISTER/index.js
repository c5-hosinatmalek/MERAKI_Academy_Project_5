...
const LOGIN= ()=>{
   ...
   const [status,setStatus]=useState(false)
    ...
    const submit=(e)=>{
        e.preventDefault();
        axios.post(`http://localhost:5000/register`,{email, password, name, country, role_id}).then((result)=>{
       ...
       <div className="containeeer" >
          ...
          <form className="form_rigister"  onSubmit={submit}>
              ...
              <div className="country_user">
                  <input placeholder="enter country" type="text" required onChange={(e)=>{setCountry(e.target.value)}} />

              </div>
              <div className="password_user">
                  <input type="password" placeholder="enter_pasword" required onChange={(e)=>{setPassword(e.target.value)}} />
              </div>
              ...
              {status?<div className="message_user"><h1>{messageUser}</h1></div>:
                    <div className="message_user"><h1>{messageUser}</h1></div>
                }
